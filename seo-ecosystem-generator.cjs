...
function extractFromHtml(html, slug, fileStats) {
  ...
  const published =
    safeParseDate(dateMeta ? dateMeta[1] : ldDate) ||
    (fileStats?.birthtime ? fileStats.birthtime.toISOString() : null) ||
    (fileStats?.mtime ? fileStats.mtime.toISOString() : null);
...
if (fs.existsSync(postMetaModulePath)) {
  ...
  if (loaded && typeof loaded === "object" && Object.keys(loaded).length > 0) {
    ...
    console.warn("⚠️ post-meta.js found but could not load entries. Will attempt to scan dist/ as fallback.");
  }
} else {
...
const allMetadata = Object.entries(postMetadata).map(([slug, meta]) => {
  ...
  const datePublished =
    safeParseDate(meta.published || meta.datePublished || meta.publishedDate) || null;
  const dateModified =
    safeParseDate(meta.modified || meta.dateModified || meta.updated) || datePublished || null;
  return {
    ...meta,
    slug,
    url: canonicalUrl,
...
const rssItems = sorted.map(post => {
  ...
  if (post.ogImage) {
    let length = 0;
    if (post.ogImage.startsWith("http")) {
      // Can't easily get remote size synchronously — fallback to 0
      length = 0;
    } else {
      length = getFileSizeSync(path.join(distDir, post.ogImage));
    }
    enclosureTag = `<enclosure url="${post.ogImage}" type="image/jpeg" length="${length}" />`;
  }
