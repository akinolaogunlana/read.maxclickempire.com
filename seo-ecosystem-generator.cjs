...
return    <item>   <title>${post.title}</title>   <link>${post.url}</link>   <description><![CDATA[${post.description}]]></description>   <pubDate>${new Date(post.published).toUTCString()}</pubDate>   <guid>${post.url}</guid>   ${categories}   </item>;
}).join("\n");

const rssFeed = <?xml version="1.0"?>   <rss version="2.0">   <channel>   <title>MaxClickEmpire Feed</title>   <link>${siteUrl}</link>   <description>Latest digital guides, tools, and growth hacks from MaxClickEmpire.</description>   <language>en-us</language>   <lastBuildDate>${new Date(posts[0].published).toUTCString()}</lastBuildDate>   ${rssItems}   </channel>   </rss>;
