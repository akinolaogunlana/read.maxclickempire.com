...
// Generate post-meta.js metadata
const metadata = {};
posts.forEach(post => {
  metadata[post.slug] = {
    title: post.title,
...
// ✅ Fix: Only write dual-compatible format ONCE
...
(async () => {
  for (const post of posts) {
    ...
    try {
      await axios.get(`https://yandex.com/indexnow?url=${encodeURIComponent(post.url)}&key=YOUR_INDEXNOW_KEY`);
