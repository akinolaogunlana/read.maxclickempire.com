...
const fs = require("fs"); const path = require("path"); const { create } = require("xmlbuilder2"); const { google } = require("googleapis"); const axios = require("axios"); const { execSync } = require("child_process");
...
// Metadata JS const metadata = {}; posts.forEach(post => { metadata[post.slug] = { title: post.title, description: post.description, image: ${siteUrl}/assets/og-image.jpg, published: post.published, canonical: post.url }; }); fs.writeFileSync(metaScriptPath, window.postMetadata = ${JSON.stringify(metadata, null, 2)};, "utf8"); console.log("✅ post-meta.js generated");
...
fs.writeFileSync(rssFile, rssFeed.trim(), "utf8"); console.log("✅ rss.xml generated with categories");
...
// Google Indexing let credentials; try { credentials = JSON.parse(fs.readFileSync("credentials.json", "utf8")); } catch (err) { console.error("❌ credentials.json is invalid or missing:", err.message); process.exit(1); }
...
async function indexUrlToGoogle(url) { try { const token = await jwt.authorize(); await axios.post("https://indexing.googleapis.com/v3/urlNotifications:publish", { url, type: "URL_UPDATED" }, { headers: { Authorization: Bearer ${token.access_token}, "Content-Type": "application/json" } });

console.log(`✅ Indexed on Google: ${url}`);

} catch (err) { console.error(❌ Failed to index ${url} on Google:, err.message); }

try { await axios.get(https://yandex.com/indexnow?url=${encodeURIComponent(url)}&key=9b1fb73319b04fb3abb5ed09be53d65e); console.log(✅ Pinged IndexNow: ${url}); } catch (err) { console.error(❌ IndexNow failed for ${url}:, err.message); } }

(async () => { for (const post of posts) { await indexUrlToGoogle(post.url); }
